...
このプロジェクトは、RAG（Retrieval-Augmented Generation）技術を使用した質問応答システムです。PDF や Word 文書などをアップロードし、その内容について自然な日本語で質問すると、関連する情報を検索して回答を生成します。
...
A: PDF、Word（.docx）、テキスト（.txt）、Markdown（.md）形式の文書が検索できます。日本語・英語に対応しています。

### Q: 何件までドキュメントを登録できますか？

A: ChromaDB は小規模データ向けのため、数百〜数千ドキュメント程度を推奨します。それ以上の場合は、Pinecone などのスケーラブルなベクトル DB への移行を検討してください。

### Q: 回答の精度を上げるにはどうすればいいですか？

A: 以下の方法を試してください:

1. チャンクサイズを調整（300〜800 の範囲）
2. 質問をより具体的にする
3. 取得ドキュメント数を増やす
4. 文書の品質を向上（構造化、見出しの追加など）

### Q: オフラインでも使えますか？

A: いいえ。Google AI Studio API を使用するため、インターネット接続が必要です。

### Q: 商用利用は可能ですか？

A: Google AI Studio の無料枠は学習・開発用途です。商用利用の場合は、Google Vertex AI への移行を検討してください。

### Q: API キーの料金はかかりますか？

A: Google AI Studio の無料枠を使用する限り、完全無料です。ただし、1 日 1,500 リクエスト、1 分 15 リクエストの制限があります。

## 今後の拡張可能性

### 短期的な改善

- ハイブリッド検索の実装（キーワード検索 + セマンティック検索）
- リランキング機能の追加
- 会話履歴の保持
- 複数ドキュメントの比較機能
...
- より大規模なベクトル DB（Pinecone, Weaviate）への移行
- マルチモーダル対応（画像、表の理解）
- ファインチューニングによる精度向上
- 本番環境向けのセキュリティ強化
